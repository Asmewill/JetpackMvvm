 ...
 */
class SystemAdapter(list:MutableList<SystemResponse>)
    :BaseQuickAdapter<SystemResponse,BaseViewHolder>(R.layout.item_system,list) {
    ...
    override fun convert(holder: BaseViewHolder, item: SystemResponse) {
        ...
        val systemChildAdapter=SystemChildAdapter()
        rv_system.adapter=systemChildAdapter
        systemChildAdapter.setList(item.children)
        systemChildAdapter.setOnItemClickListener{adapter,view,positon->
